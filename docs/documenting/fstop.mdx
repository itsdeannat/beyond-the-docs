...
Explore how I designed a documentation workflow for an API and SDK, including OpenAPI automation, reference documentation, and code examples.
...
I initially had the Speakeasy configuration file in both the Fstop API repo and the SDK repo. Maintaining the configuration in two places created unnecessary duplication and caused the repositories to fall out of sync. 

I simplified the workflow by keeping the Speakeasy configuration in the SDK repo and configuring it to pull the OAS directly from the API repo:
...
The Python SDK is auto-generated from the OpenAPI spec using Speakeasy. Rather than creating separate SDK reference documentation, I included Python SDK examples alongside cURL examples in the API feature guides. This serves both audiences: HTTP users see cURL and SDK users see how to accomplish the same task using Python.
